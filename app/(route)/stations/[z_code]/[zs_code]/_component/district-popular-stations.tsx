...
// zs_code별 인기 충전소 조회 함수
async function getPopularStationsByZsCode(zsCode: string, limit: number) {
  try {
...
const DistrictPopularStations = async (props: Props) => {
  const supabase = await createSupabaseServerClientReadOnly();
  const poplurStations = await getPopularStationsByZsCode(props.zsCode, 5);
  const response = await supabase
    .from("stations")
    .select("*, chargers(*)")
    .eq("zs_code", props.zsCode)
    .in(
      "id",
...
export default DistrictPopularStations;
